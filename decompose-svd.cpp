 ...
 * The input Numpy zip file needs to contain 4 Numpy arrays for the density, pressure, and velocity in x,y
 * directions. 
 * 
...
int main(int argc, char **argv)
{
    ...
    /*
     * the Numpy zip file, which contains the original fluid data (density, pressure, 
     * velocity in x,y directions)
     * afterward, each numpy array from the zip will be iterated for the SVD computation.  
     */
    ...
    for (auto &iter : my_npz)
    {
        ...
        /*
         * Send the elements from the local arrays of Vt on parallel processes to the 
         * global array Vt on the root process
         */
        ...
        for (int r = 0; r < p; r += Nb, sendrow = (sendrow + 1) % procrows)
        {
            ...
            for (int c = 0; c < M; c += Mb, sendcol = (sendcol + 1) % proccols)
            {
                ...
                if (mpiroot)
                {
                    // Receive the submatrix sent from parallel processes
        ...
        if (mpiroot)
        {
            ...
            */
            ...
            string compressedPath = orgPath + "_" + to_string(compression_ratio) + "/";
            string zipName = compressedPath + compressedFileName;

            if (!fs::is_directory(compressedPath) || !fs::exists(compressedPath))
            {
                fs::create_directory(compressedPath);
            }
            string mode = iter.first == "denArr" ? "w" : "a";

            cnpy::npz_save(zipName, iter.first + "_u", &finalU[0], {NLong, countLong}, mode);
            ...
            /*
             * Save the maximum elapsed time for SVD in a text file
             */
            cout << svd << endl;
