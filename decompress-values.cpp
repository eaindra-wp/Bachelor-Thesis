/*
 * The compressed fluid data for all 4 properties, which are saved in the same Numpy archive, 
 * can be decompressed iteratively.
 * The input Numpy zip should contain 12 Numpy arrays in total, which are
 * 3 Numpy arrays (U,Sigma,Vt) for each fluid property data.
 *
...
int main(int argc, char **argv)
{
    ...
    /*
     * The Numpy zip contains twelve Numpy arrays: three arrays Sigma,U,Vt for each fluid property. 
     * In each iteration, Sigma,U,Vt for density,pressure,velX,velY fluid properties will
     * be taken to decompress the fluid data. 
     */
    string fname(argv[1]);
    cnpy::npz_t my_npz = cnpy::npz_load(fname);
    int k = 3;
    auto start = my_npz.begin();
    auto midpoint = next(my_npz.begin(), k);
    while (k <= my_npz.size())
    {
        ...
        if (p > 0 && pb > proccols)
        {
            ...
            */
            ...
            if (mpiroot)
            {
                ...
                /*
                 * Here, the decompressed result of density, pressure, velocity in x and y directions will be
                 * saved in the same Numpy zip archive. 
                 * The density values will be decompressed first, so a new Numpy zip archive will be created first
                 * if the header of the compressed Numpy arrays (U,Sigma,Vt) is 'denArr'. 
                 ...
                 * If the decompressed result is for other properties, then the result results will be appended to 
                 * the existing Numpy zip archive as new Numpy arrays. 
                 */
                ...
                string mode = k == 3 ? "w" : "a";
                cnpy::npz_save(zipName, compressedFileName, &final_result[0], {NLong, MLong}, mode);
                ...
                */
                ...
                string timeFileName = orgPath + "_decompression_timetakens";
